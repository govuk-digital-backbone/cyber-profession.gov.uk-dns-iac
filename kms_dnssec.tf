...
output "secondary_dnskey_record" {
  ...
  description = "The secondary DNSKEY record for cyber-profession.gov.uk"
}

output "ds_records" {
  value       = "${local.domain}. ${local.low_ttl} IN DS ${aws_route53_key_signing_key.primary.ds_record}\n${local.domain}. ${local.low_ttl} IN DS ${aws_route53_key_signing_key.secondary.ds_record}"
  description = "The DS records for cyber-profession.gov.uk"
}

# =====

resource "aws_route53_key_signing_key" "primary" {
  hosted_zone_id             = aws_route53_zone.cyberprofessiongovuk.id
...
resource "aws_route53_hosted_zone_dnssec" "cyberprofessiongovuk" {
  depends_on = [
